...
func _physics_process(delta: float) -> void:
	...
	if input_dir.length() > 0.0:
		...
		# O vetor de movimento é transformado pela rotação do Player (que é a rotação da câmera)
	...
	# O Player (e a câmera que é filha dele) gira em torno do eixo Y
	var rotation_input = Input.get_action_strength("ui_right") - Input.get_action_strength("ui_left")
	
	if rotation_input != 0.0:
		# Rotação suave do Player (e da câmera que é filha dele)
		# O valor 60.0 é um multiplicador para tornar a rotação mais perceptível
